...
def cluster(feature, true_labels, cluster_num):
    predict_labels, centers = kmeans(X=feature, num_clusters=cluster_num, distance="euclidean", device="cuda")
    acc, nmi, ari, f1 = eva(true_labels, predict_labels.numpy(), show_details=False)
    return 100 * acc, 100 * nmi, 100 * ari, 100 * f1, predict_labels.numpy(), centers
...
#     return infoNEC
def hard_sample_aware_infoNCE(S, M, node_num):
...
def dclInstance(S, M, node_num):
    pos_neg = M * torch.exp(S * 1)
    pos = torch.cat([torch.diag(S, node_num), torch.diag(S, -node_num)], dim=0)
    pos = torch.exp(pos * 1)
    neg = (torch.sum(pos_neg, dim=1) - pos)
    dcl = (-torch.log(pos).sum() + torch.log(neg).sum())/(2*node_num)
    return dcl

def dclCluster(c1, c2, temperature):
    logits = torch.matmul(c1, c2.t())
    diag = torch.diag(logits)
    pos = diag/temperature
    neg_logits = logits - diag.unsqueeze(1)
    neg = torch.exp(neg_logits/temperature).sum(dim=1)
    dclCluster = -torch.mean(torch.log(pos))+torch.mean(torch.log(neg))
    return dclCluster

def dclCluster_v1(c1, c2):
    logits = torch.matmul(c1, c2.t())
    diag = torch.diag(logits)
    pos = diag
    neg_logits = logits - diag.unsqueeze(1)
    neg = torch.exp(neg_logits).sum(dim=1)
    dclCluster = -torch.mean(torch.log(pos))+torch.mean(torch.log(neg))
    return dclCluster

def infoNCEInstance(z1, z2, temperature):
    logits = torch.matmul(z1, z2.t())
    diag = torch.diag(logits)
    pos = diag/temperature
    neg_logits = logits - diag.unsqueeze(1)
    neg = torch.exp(neg_logits/temperature).sum(dim=1)
    infoNCEInstance = -torch.mean(torch.log(pos/(pos+neg)))
    return infoNCEInstance

def infoNCEloss(z1, z2, temperature):
    logits = torch.matmul(z1, z2.t())
    diag = torch.diag(logits)
    pos = diag/temperature
    neg_logits = logits - diag.unsqueeze(1)
